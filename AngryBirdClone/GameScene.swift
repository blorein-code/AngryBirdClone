...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        */
        ...
        //Physics Body - Ekrana belli sınırlar çizmek için
        self.physicsBody = SKPhysicsBody(edgeLoopFrom: frame)
        self.scene?.scaleMode = .aspectFit
        self.physicsWorld.contactDelegate = self
    
        //Bird
        //GameScene içerisinde oluşturulanları tanımladığımız alan
        bird = childNode(withName: "bird") as!  SKSpriteNode
        
        let birdTexture = SKTexture(imageNamed: "bird")
        //Bir SKPhysicsBody oluşturduk ki yer çekimi, çarpma hasarı gibi olaylardan etkilenebilsin.
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        //oyun başlamadıysa
        if gameStarted == false  {
            //dokunulan yeri aldık
            if let touch = touches.first {
                let touchLocation = touch.location(in: self)
                //bunu bir node'a verdik
                let touchNodes = nodes(at: touchLocation)
                //bi yere tıklandıysa
                if touchNodes.isEmpty == false {
                    for node in touchNodes {
                        //tıklanan noktaya kuşu verdik
                        if let sprite = node as? SKSpriteNode {
                            if sprite == bird {
                                bird.position = touchLocation
                            }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if let birdPhysicsBody = bird.physicsBody {
            //kuşun x hızı, y hızı, açısal hızı ve oyun başlamış mı kontrollerine göre işlemler yapıldı.
